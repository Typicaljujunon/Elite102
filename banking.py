...
def logged_in(username, perms):
    ...
    balance_label = tk.Label(logged_in_window, text="", font=("Calibri", 12))
    balance_label.grid(row=2, columnspan=2)
    ...
    def update_balance_label():
        ...
        cursor.execute("SELECT balance FROM bankaccounts WHERE username = %s", (username,))
        current_balance = cursor.fetchone()[0]
        balance_label.config(text="Your current balance is: $" + str(current_balance))
    ...
    operations = ["Check Balance", "Change Information", "Deposit", "Withdrawal", "Transfer", "Change Name"]
    if perms == 'admin':
    ...
    def execute_operation():
        ...
        elif operation == "Change Information":
...
def logout():
    ...
    thank_you_label = tk.Label(thank_you_window, text="Thank you for using our banking app!", font=("Calibri", 16))
    thank_you_label.pack(pady=20)
    logout_button = tk.Button(thank_you_window, text="Close", command=thank_you_window.destroy, font=("Calibri", 12))
    ...
    # Button to logout
    logout_button = tk.Button(logged_in_window, text="Logout", command=logout, font=("Calibri", 12))
